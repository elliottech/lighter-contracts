...
/// NOTE: NO AUDIT - PLACEHOLDER (auto generated by gnark)
/// @title zkLighter Desert Mode Verifier Contract
/// @author zkLighter Team
/// @dev This contract is generated by gnark for zkLighter desert circuit's verification key with few modifications.
///   - Unused proof compression functions are removed.
///   - The contract is made upgradeable.
contract DesertVerifier is IDesertVerifier {
  // ------ UPGRADAABLE START ------
  ...
  uint256 private constant VK_QL_COM_X = 17871376003628267780212758562548794301080530931478560805785572371807442782346;
  uint256 private constant VK_QL_COM_Y = 3399584496846113797017870211794403654605317553524416935174103793154757819895;
  uint256 private constant VK_QR_COM_X = 4345413216309809935674610227931600736245672677786837663175281131734802687935;
  uint256 private constant VK_QR_COM_Y = 18785871212426188231229481816940679699849084655401019202076189512471514342419;
  uint256 private constant VK_QM_COM_X = 10928507796005133436448578894892174669458921597571218147084563656491344230485;
  uint256 private constant VK_QM_COM_Y = 13755231146930112815884762327003738544473222763863949881643051429249599029699;
  uint256 private constant VK_QO_COM_X = 2312258175105812571206600018002199632312522529846248713093959307921664926527;
  uint256 private constant VK_QO_COM_Y = 139753637491959023839592098255597266570849579981737810420973669213071728614;
  uint256 private constant VK_QK_COM_X = 10952421172845450772689537395862385002691646348729181014039766464632604557211;
  uint256 private constant VK_QK_COM_Y = 11173611551851667496593745133944282314093689679703235742453788239879628912769;

  uint256 private constant VK_S1_COM_X = 3440932044316682041014175411626222105223527911587244152418357917887917564696;
  uint256 private constant VK_S1_COM_Y = 19912824767411275767471593778799127401857032401735873579334768413876910330452;

  uint256 private constant VK_S2_COM_X = 16712732795813444438330479948168254262323087534413646777574630449021065828311;
  uint256 private constant VK_S2_COM_Y = 14882145513270948189426477309762891957002095901844343772760426525364875225997;

  uint256 private constant VK_S3_COM_X = 4159134974389863121481883747714209707106813534758972928299757492954230850963;
  uint256 private constant VK_S3_COM_Y = 1637290957750351011296296987739905843443376061455280293410924084156643638709;

  uint256 private constant VK_COSET_SHIFT = 5;

  uint256 private constant VK_QCP_0_X = 21058401064056884971713634733987838084984440187550576136194242110557981451923;
  uint256 private constant VK_QCP_0_Y = 1685776880211398673983354729370119752630210329810610948644505185050660692183;

  uint256 private constant VK_INDEX_COMMIT_API0 = 10738062;
  ...
  function Verify(bytes calldata proof, uint256[] calldata public_inputs) public view returns (bool success) {
    assembly {
      ...
      function batch_verify_multi_points(aproof) {
        ...
        mPtr := add(mPtr, 0x40)
        point_mul_calldata(folded_points_quotients, add(aproof, PROOF_BATCH_OPENING_AT_ZETA_X), mload(add(state, STATE_ZETA)), mPtr)
      ...
      function compute_commitment_linearised_polynomial_ec(aproof, s1, s2) {
        ...
        point_mul(add(state, STATE_LINEARISED_POLYNOMIAL_X), mPtr, calldataload(add(aproof, PROOF_L_AT_ZETA)), add(mPtr, 0x40))

        mstore(mPtr, VK_QR_COM_X)
        mstore(add(mPtr, 0x20), VK_QR_COM_Y)
        point_acc_mul(add(state, STATE_LINEARISED_POLYNOMIAL_X), mPtr, calldataload(add(aproof, PROOF_R_AT_ZETA)), add(mPtr, 0x40))
        ...
        point_acc_mul(add(state, STATE_LINEARISED_POLYNOMIAL_X), mPtr, calldataload(add(aproof, PROOF_O_AT_ZETA)), add(mPtr, 0x40))
